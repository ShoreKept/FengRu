...
from models.model_clam_ypy import CLAM_MB, CLAM_SB #这里model换一下
...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):  # 对单个 WSI 进行推理
    ...
    return ids, preds_str, probs, A  # ids：预测的类别索引。preds_str：预测类别名称。
 #/////////
def load_params(df_entry, params):   # 从 DataFrame 读取参数，更新 params 字典
...
def parse_config_dict(args, config_dict):  #  从命令行参数 args 读取实验编号和 Patch 重叠率，并更新 config_dict
...
if __name__ == '__main__':
    ...
    if preset is not None:
        preset_df = pd.read_csv(preset)    #   这里 从 CSV 文件里读取参数，覆盖默认参数
    ...
    if data_args.process_list is None:
        if isinstance(data_args.data_dir, list): # 如果 data_dir 是列表，就合并所有目录下的切片。
            slides = []
        ...
        # 如果 process_list 指定了 CSV 文件，就直接从 CSV 加载要处理的切片列表
    mask = df['process'] == 1
...
# //////////////
    ...
    for i in tqdm(range(len(process_stack))):
        slide_name = process_stack.loc[i, 'slide_id'] # 读取切片信息slide_id!!!!!
        ...
        if isinstance(data_args.data_dir, str): # WSI路径是字符串还是字典？
            slide_path = os.path.join(data_args.data_dir, slide_name)  ##这个部分修改？
        ...
        if not os.path.isfile(slide_path):
            print(f"Warning: Slide file {slide_path} not found, skipping...")
            continue  # 直接跳过当前 slide，避免报错
        ...
        seg_params = load_params(process_stack.loc[i], seg_params) # 加载分割参数
        filter_params = load_params(process_stack.loc[i], filter_params)
        vis_params = load_params(process_stack.loc[i], vis_params)

        keep_ids = str(seg_params['keep_ids']) # keep_ids 代表需要保留的组织区域 ID
        if len(keep_ids) > 0 and keep_ids != 'none':
            seg_params['keep_ids'] = np.array(keep_ids.split(',')).astype(int)
        else:
            seg_params['keep_ids'] = []

        exclude_ids = str(seg_params['exclude_ids']) # 代表要去除的ID
        ...
        #用于热力图可视化的实际补丁大小应为：补丁大小 × 下采样因子 × 自定义下采样因子。
        ...
        features_path = os.path.join(r_slide_save_dir, slide_id+'.pt') # 特征
        h5_path = os.path.join(r_slide_save_dir, slide_id+'.h5')
    
#/////////////////////////////////////////
        ##### check if h5_features_file exists ######
        ## 检查h5_features_file存在性
...
#            process_stack.to_csv('heatmaps/results/{}.csv'.format(exp_args.save_exp_code), index=False)
        base_dir = '/home/graduate2024/code/zxb/CLAM-master/heatmaps/results'
        os.makedirs(base_dir, exist_ok=True)  # 确保目录存在

        if data_args.process_list is not None:
            save_path = os.path.join(base_dir, '{}.csv'.format(data_args.process_list.replace('.csv', '')))
        else:
            save_path = os.path.join(base_dir, '{}.csv'.format(exp_args.save_exp_code))

# 保存文件
        process_stack.to_csv(save_path, index=False)#new
...
####################
        ...
        with h5py.File(save_path, 'r') as file:
            ...
            dset = file['attention_scores'] # 注意力分数
            coord_dset = file['coords']
            scores = dset[:]
            coords = coord_dset[:]
